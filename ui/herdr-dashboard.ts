...
export interface HerdrDashboardOptions {
  onQuit?: () => void
  onBack?: () => void
  onSave?: (snapshot: HerdrConfigSnapshot, values: Record<string, string>) => Promise<HerdrConfigSaveResult>
...
function settingDetail(
  ...
  if (definition.choices) {
    ...
    pushLine(chunks)
    const range =
      definition.min !== undefined && definition.max !== undefined ? `${definition.min}–${definition.max}` : 'numeric'
...
export function createHerdrDashboard(
  ...
  const configPathLabel = new TextRenderable(renderer, {
    ...
    width: 8,
    height: 1,
    content: new StyledText([fg(theme.border)('⟨'), fg(theme.info)('path'), fg(theme.border)('⟩ ')])
  })
  ...
  const selectedDefinition = (): HerdrSettingDefinition =>
    (settingList.getSelectedOption()?.value as HerdrSettingDefinition | undefined) ?? selectedCategory().settings[0]!
  ...
  const updateCategoryGrid = (): void => {
    ...
    const availableWidth = Math.max(tileWidth, targetSettingsPanelWidth() - 2 - CATEGORY_GRID_PADDING_X * 2)
    const columns = Math.max(1, Math.floor((availableWidth + CATEGORY_TILE_GAP) / (tileWidth + CATEGORY_TILE_GAP)))

    categoryLayoutRows = []
    for (let index = 0; index < HERDR_SETTING_CATEGORIES.length; index += columns) {
      categoryLayoutRows.push(HERDR_SETTING_CATEGORIES.slice(index, index + columns).map((_, offset) => index + offset))
    }
  ...
  const moveCategoryHorizontal = (direction: -1 | 1): void => {
    selectCategoryIndex(Math.max(0, Math.min(HERDR_SETTING_CATEGORIES.length - 1, selectedCategoryIndex + direction)))
  }
  ...
  const save = async (): Promise<void> => {
    ...
    try {
      ...
      refreshFooter()
      setStatus(
        result.backupPath ? 'Saved · backup updated · Press y to reload config.' : 'Saved · Press y to reload config.',
        'warning'
      )
    } catch (error) {
      setStatus(error instanceof Error ? error.message : String(error), 'error')
