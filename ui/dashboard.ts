...
type DashboardRow =
  | { kind: "file"; file: FileAnalysis }
  | { kind: "unused"; analysis: ProjectAnalysis };
...
function detailForFile(file: FileAnalysis): StyledText {
  ...
  if (file.imports.length === 0) {
    ...
    for (const imported of file.imports) {
      const alias =
        imported.local && imported.local !== imported.imported
          ? ` as ${imported.local}`
          : "";
      const typeLabel = imported.typeOnly ? "type " : "";
      pushLine(
        chunks,
        `  ↓ ${typeLabel}${imported.imported}${alias}`,
        theme.text,
      );
  ...
  if (file.diagnostics.length === 0) {
    ...
    for (const diagnostic of file.diagnostics) {
      ...
      const color =
        diagnostic.severity === "error" ? theme.error : theme.warning;
      const location = diagnostic.line ? `:${diagnostic.line}` : "";
      pushLine(
        chunks,
        `  ${marker} ${diagnostic.code}${location}`,
        color,
      );
...
function summaryText(analysis: ProjectAnalysis): StyledText {
  ...
  return new StyledText([
    fg(theme.textMuted)("ROOT "),
    fg(theme.text)(analysis.root),
    fg(theme.border)("   │   "),
    fg(theme.info)(`${totals.files} files`),
    fg(theme.border)("   "),
    fg(theme.text)(`${totals.imports} imports`),
    fg(theme.border)("   "),
    fg(theme.text)(`${totals.exports} exports`),
    fg(theme.border)("   "),
    fg(issueColor)(`${totals.errors + totals.warnings} diagnostics`),
    fg(theme.border)("   "),
...
function displayName(
  ...
  const tableRow =
    row.kind === "file" ? analysisRow(row.file) : unusedExportsRow(analysis);
...
export function createDiagnosisDashboard(
  ...
  const brand = new TextRenderable(renderer, {
    ...
    content: new StyledText([
      bold(fg(theme.accent)("BUTTLER")),
      fg(theme.textMuted)("  /  CODE DIAGNOSIS"),
    ]),
  });
  ...
  const filterLabel = new TextRenderable(renderer, {
    ...
    height: 1,
    content: new StyledText([
      bold(fg(theme.textMuted)("FILTER  ")),
      fg(theme.accent)("/"),
    ]),
  });
  const filterInput = new InputRenderable(renderer, {
    id: "filter",
    flexGrow: 1,
    placeholder: "type to narrow files",
  ...
  const filePanel = new BoxRenderable(renderer, {
    ...
    title: ` FILES · ${analysis.files.length} `,
  ...
  const updateDetail = (row: DashboardRow | undefined): void => {
    ...
    detail.content =
      row.kind === "file"
        ? detailForFile(row.file)
        : detailForUnused(row.analysis);
    detailScroll.scrollTo(0);
  };

  const refreshRows = (preferredPath?: string): void => {
    const selected = fileList.getSelectedOption()?.value as
      | DashboardRow
      | undefined;
    const rememberedPath =
      preferredPath ?? (selected?.kind === "file" ? selected.file.path : "");
    ...
    const rememberedIndex = filteredRows.findIndex(
      (row) => row.kind === "file" && row.file.path === rememberedPath,
    );
...
export async function runInteractiveDiagnosis(
  target: string,
