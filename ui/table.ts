...
export function createTableSeparator(columns: TableColumn[]): string {
  return columns
    .map((column) => "⎺".repeat(Math.max(1, column.width)))
    .join(" ");
}
...
export function analysisFileWidth(
...
export function formatAnalysisTable(
  analysis: ProjectAnalysis,
  fileWidth?: number,
): string {
  const longestPath = analysis.files.reduce(
    (longest, file) => Math.max(longest, getVisualWidth(file.path)),
    0,
  );
  const resolvedFileWidth =
    fileWidth ?? Math.max(35, Math.min(70, longestPath));
  const rows = [
    ...analysis.files.map(analysisRow),
    unusedExportsRow(analysis),
  ];
